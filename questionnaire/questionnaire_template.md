...
Full item-level coding is in `docs/data_dictionary.md`. Translate and back-translate all sections into Bahasa Indonesia before piloting (Section 4.7 of `docs/PROTOCOL.md`).
...
## Referral pathway (mandatory)

Any participant screening positive for likely cognitive impairment (per MMSE/MoCA cutoff) or likely decline (per IQCODE cutoff) **must** be referred to the site's geriatric/neurology clinical pathway for full diagnostic follow-up, per `docs/PROTOCOL.md` Section 4.9. Document the referral in the case report form.
